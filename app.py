...
class Seats(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    train_id = db.Column(db.String(100), nullable=False)
...
def stationdash():
    if "admin" in session:
        ...
        scheduled_trains = Seats.query.count()
        return render_template(
            "station_dash.html", data=[trains, scheduled_trains, users]
        )
    else:
        flash("Session Expired", "error")
...
@app.route("/scheduledtrainlist")
def scheduledtrainlist():
    if "admin" in session:
        data = Seats.query.all()
        return render_template("scheduled_train_list.html", data=data)
    else:
        flash("Session Expired", "error")
        return redirect(url_for("stationlog"))
...
def userdash():
    if "user" in session:
        ...
        return render_template("user_dash.html", data=[trains, history])
    else:
        flash("Session Expired", "error")
...
def available_train():
    if "user" in session:
        ...
        train_ids = [seat.train_id for seat in seats]
        avail = Train.query.filter(Train.id.in_(train_ids)).all()
        return render_template("availtrain_list.html", data=avail, seats=seats)
    else:
        flash("Session Expired", "error")
        return redirect(url_for("userlog"))
...
def Book_train():
    if "user" in session:
        ...
        return render_template(
            "Book_train.html", location=station_locations, train=trains_avialable
        )
    else:
        flash("Session Expired", "error")
        return redirect(url_for("userlog"))
...
def user_reserve():
    if "user" in session:
        ...
        trains_avialable = Seats.query.all()
        return render_template(
            "user_reserve.html", location=station_locations, train=trains_avialable
        )
    else:
        flash("Session Expired", "error")
        return redirect(url_for("userlog"))
...
def cancel(id):
    if "user" in session:
        ...
        if da < current:
            ...
            change_seats = Seats.query.filter_by(train_id=train, date=user_date).first()
    ...
    if "admin" in session:
        if request.method == "POST":
            ...
            existing_seats = Seats.query.filter_by(
                train_id=train_id, date=date_str
            ).first()
            if existing_seats:
                flash("Train already scheduled for that date", "error")
                return redirect(url_for("stationdash"))
            train = Train.query.filter_by(id=train_id).first()
            seats = Seats(train_id=train_id, date=date_str, seats_count=train.seats)

...
app = Flask(__name__)                      #object created for running flask app
app.config['SECRET_KEY'] = os.urandom(16)   #used for sessions
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///rail_db.db"
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False
app.config['SESSION_COOKIE_NAME'] = 'login-system'
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(minutes=5)       #google auth session expires after 5 mins
...
def book_ticket():
    if request.method == 'POST':
        if 'user' in session:
            ...
            if name and email: 
                seats = Seats.query.filter_by(train_id=train_id).all()   #fetching all seats of that train(different dates but same train so all seats of different dates are fetched)  
                for i in seats:                           #for each seat in the train.Same train for different dates so this is done
                    if i.date == date:                    #i.date is the train's date and date is user's
                        id_train = i.id                   #if seat available for the date entered by user then that train'sid is taken and stored in id_train  
                        break
                    else:
                        continue
                if id_train != 0:
                    seatss = Seats.query.filter_by(id=id_train).first()       #seats from train of that date is fetched
...
def cancel(id):
    if 'user' in session:
        ...
        da = datetime.datetime.strptime(user_date,'%Y-%m-%d')   #date of booking
